...
public class MemberAddActivity extends BaseActivity
{
	...
	private static ArrayList<Relation> relations;			//関係性クラス配列
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		relationDao = new RelationDao(getApplicationContext());
	...
	private void edit()
	{
		...
		Member member = memberDao.selectMemberDetail(db, id);
		db.close();
		...
		if (member.getBirth() != null) {
			...
			String[] date = Utils.separateDate(member.getBirth(), "/");
	 ...
	 * @access private
	 */
	private void save(ArrayList<Relation> relations, Integer id)
	{
		TextView editTextName = (TextView) findViewById(R.id.editTextName);
		...
		validate.set();
		validate.require.check(name, "名前", "");
		...
		Relation relation = relations.get(relation_position);
	 ...
	 * エラー用ダイアログ表示
	 * 
	 * @param ArrayList<String> msgs メッセージ配列
	 * @return void
	 * @access private
	 */
